...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_tag'])) {
    ...
    if (empty($name)) {
        ...
        $errors[] = "Tag name must be at least 3 characters long.";
    }

    $slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $name), '-'));

    if (empty($errors)) {
        $check = $conn->prepare("SELECT id FROM tags WHERE name = ? OR slug = ?");
        $check->bind_param("ss", $name, $slug);
        $check->execute();
        $check->store_result();

        if ($check->num_rows > 0) {
            $_SESSION['error'] = "Tag with this name already exists.";
        } else {
...
$tags = [];
$result = $conn->query("SELECT * FROM tags ORDER BY id ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_tag'])) {
    ...
    if (empty($name) || strlen($name) < 3) {
        $_SESSION['error'] = "Invalid tag name.";
    } else {
        $slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $name), '-'));

        // Check if another tag with same name or slug exists (exclude current tag ID)
        $check = $conn->prepare("SELECT id FROM tags WHERE (name = ? OR slug = ?) AND id != ?");
        $check->bind_param("ssi", $name, $slug, $id);
        $check->execute();
        $check->store_result();

        if ($check->num_rows > 0) {
            $_SESSION['error'] = "Tag with this name or slug already exists.";
        } else {
            ...
            if ($stmt->execute()) {
                ...
                $_SESSION['error'] = "Error updating tag: " . $stmt->error;
            }
        }
    }
    header("Location: " . $_SERVER['PHP_SELF']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $check = $conn->prepare("SELECT id FROM tags WHERE id = ?");
                            ...
                            <div class="modal fade" id="editTagModal<?= $tag['id'] ?>" tabindex="-1" aria-labelledby="editTagLabel<?= $tag['id'] ?>" aria-hidden="true">
                                <div class="modal-dialog">
                                    <form method="post" action="">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editTagLabel<?= $tag['id'] ?>">Edit Tag</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    <label for="edit_name_<?= $tag['id'] ?>" class="form-label">Tag Name</label>
                                                    <input type="text" class="form-control" id="edit_name_<?= $tag['id'] ?>" name="edit_name" value="<?= htmlspecialchars($tag['name']) ?>" required>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="submit" name="update_tag" class="btn btn-success">Update</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
